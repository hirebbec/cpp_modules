...
void ClapTrap::attack(const std::string &target) {
    if (energyPoints > 0 && hitPoints > 0) {
        energyPoints--;
        std::cout << name << " attack " << target << ", causing " << attackDamage << " points of damage"
                  << std::endl;
    } else{
...
void ClapTrap::takeDamage(unsigned int amount) {
    if (energyPoints > 0 && hitPoints > 0) {
        energyPoints--;
        hitPoints -= amount;
        std::cout << name << " takes " << amount << " damage, now he has " << hitPoints << " hitPoints" << std::endl;
    } else{
        std::cout << name << " has no hitPoints or energyPoints" << std::endl;
    }
}
...
void ClapTrap::beRepaired(unsigned int amount) {
    if (energyPoints > 0 && hitPoints > 0) {
        energyPoints--;
        hitPoints += amount;
        std::cout << name << " repaired for " << amount << " hitPoints, now he has " << hitPoints << " hitPoints" << std::endl;
    } else{
        std::cout << name << " has no hitPoints or energyPoints" << std::endl;
    }
}